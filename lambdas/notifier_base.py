...
class EmailNotifierHandler(abc.ABC):
    ...
    def _send_result(self, event_id: str, *, status: str, error: str = None):
    ...
    def _send_email(self, *, to: str, data: dict, event_type: str):
        action = event_type.split(".")[-1].replace("_", " ").capitalize()
        self._get_ses_client().send_email(
            Source=os.environ["SES_FROM_EMAIL"],
            Destination={"ToAddresses": [to]},
            Message={
                "Subject": {"Data": self.build_subject(data, action)},
                "Body": {"Text": {"Data": self.build_body(data, action)}},
            },
        )
    ...
    def handle(self, event, context):
        for record in event["Records"]:
            ...
            try:
                ...
                logger.info(
                    "Processing event id=%s type=%s tenant=%s record_id=%s",
                    event_id,
                    event_type,
                    body.get("tenant_id"),
                    data.get("id"),
                ...
                if not recipient:
                    logger.warning(
                        "No recipient email in event_id=%s, skipping email.",
                        event_id,
                    )
                else:
                ...
                if event_id:
                    self._send_result(event_id, status="failed", error=str(exc))
